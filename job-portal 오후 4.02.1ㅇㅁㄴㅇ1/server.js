...
app.post('/api/create-post', authenticateToken, (req, res) => {
  ...
  const query = `
    INSERT INTO job_registration (phone_number, title, job_category, rate, date, start_time, end_time, location, personnel, description)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  ...
  db.query(query, [phone_number, title, job_category, rate, date, start_time, end_time, location, personnel, description], (err, result) => {
    if (err) {
      ...
      res.status(500).json({ error: '게시글 작성 중 오류가 발생했습니다.', details: err });
    } else {
      res.status(201).json({ success: '게시글이 성공적으로 작성되었습니다.' });
    }
  });
});
